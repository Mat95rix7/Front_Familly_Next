...
//     `${pere.first_name} ${pere.last_name}`.toLowerCase().includes(searchTerm.toLowerCase())  );
...
//       <h1 className="text-3xl font-bold text-cyan-300 mb-8 text-center">Liste des Familles</h1>
//             <div className="mb-6">
...
//         {filteredfamilles.length > 0 ? filteredfamilles.map((pere) => (
//           <div key={pere.id} className="card bg-gray-900 text-white shadow-lg rounded-2xl h-full flex flex-col items-center p-6">
//             <Link href={`/familles/${pere.id}`} className="flex flex-col items-center text-white hover:text-cyan-300 transition">
//               <Image
...
//               />
//               <h4 className="text-xl font-bold mb-1">{pere.last_name?.charAt(0)}. {pere.first_name} </h4>
//               <span className="inline-block bg-cyan-400 text-gray-900 font-semibold px-3 py-1 rounded-full mt-1">Voir la famille</span>
//             </Link>
//           </div>
//         )) : (
//           <div className="col-span-full">
//             <div className="bg-yellow-100 text-yellow-800 rounded p-4 text-center">Aucune Famille trouvée.</div>
//           </div>
//         )}
//       </div>
//     </div>
...
export default function PeresList() {
  ...
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [groupBy, setGroupBy] = useState(false); // false = liste simple, true = groupé
